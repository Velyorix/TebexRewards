@php
    $formAction = route($formActionRoute);
    $pollMs = max(5000, min(300000, (int) ($pollSeconds ?? 10) * 1000));
    $tableClass = ($tableDensity ?? '') === 'table-sm' ? 'table table-sm mb-0' : 'table mb-0';
    $lbRootClass = 'card tebexrewards-lb mb-4';
    if (($tableDensity ?? '') === 'table-sm') {
        $lbRootClass .= ' tebexrewards-lb--compact';
    }
@endphp

@push('styles')
    <link rel="stylesheet" href="{{ plugin_asset('tebexrewards', 'css/leaderboard.css') }}">
@endpush

<div class="card mb-4">
    <div class="card-body">
        <form method="GET" action="{{ $formAction }}" class="row gy-2 gx-3 align-items-end">
            <div class="col-md-4">
...
<div class="{{ $lbRootClass }}" id="tebexrewards-leaderboard-root"
...
<script>
    (function () {
        ...
        const pollMs = parseInt(root.getAttribute('data-poll-ms') || '10000', 10) || 10000;
